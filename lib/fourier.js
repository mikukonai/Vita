...
DCT_2D.prototype = {
    ...
    dct: function(input) {
        ...
        if(size === 8) {
            for(let y = 0; y < size; y++) {
                output[y] = DCT_8_fast(input[y]);
            }
        }
        else {
            for(let y = 0; y < size; y++) {
                output[y] = this.dct_obj.dct(input[y]);
            }
        }
        ...
        for(let x = 0; x < size; x++) {
            ...
            let dctcol;
            if(size === 8) {
                dctcol = DCT_8_fast(col);
            }
            else {
                dctcol = this.dct_obj.dct(col);
            }
            // 结果写回temp1
            for(let y = 0; y < size; y++) {
                output[y][x] = dctcol[y];
    ...
    idct: function(input) {
        ...
        for(let x = 0; x < size; x++) {
            ...
            // 如果宽度为8，则使用8点快速IDCT
            if(size === 8) {
                col = IDCT_8_fast(dctcol);
            }
            else {
                col = this.dct_obj.idct(dctcol);
            }
            // 结果写回output
            for(let y = 0; y < size; y++) {
                output[y][x] = col[y];
        ...
        if(size === 8) {
            for(let y = 0; y < size; y++) {
                output[y] = IDCT_8_fast(output[y]);
            }
        }
        else {
            for(let y = 0; y < size; y++) {
                output[y] = this.dct_obj.idct(output[y]);
            }
        }
